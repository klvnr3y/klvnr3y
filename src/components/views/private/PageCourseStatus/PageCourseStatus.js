...
export default function PageCourseStatus() {
  ...
  const { isLoading: isLoadingModule } = GET(
    ...
    (res) => {
      ...
      if (res.data) {
        let data = [];
        res.data.map((itemModule, indexModule) => {
          let questionCount = 0;
          let answerCount = 0;
          let lessons = [];

          itemModule.lessons.map((itemLesson, indexLesson) => {
            let status = "Up Next";
            let questionsCount = itemLesson.questions.length;
            let userAnswersCount = itemLesson.user_answers.length;
            if (indexLesson === 0) {
        ...
        Highcharts.chart("div_module_status", {
          ...
          tooltip: {
            headerFormat: "<table>",
            pointFormat:
              '<tr><td style="padding:0">{point.name}: </td>' +
        ...
        <Col xs={24} sm={24} md={24} lg={24} xl={7}>
          ...
          >
            <Collapse.Panel
              ...
              className="accordion bg-darkgray-form m-b-md border "
            >
              <div id="div_module_status" style={{ height: "230px" }} />
            </Collapse.Panel>
          </Collapse>
        </Col>
      </Row>
